...
console.log(token)
  try{
    ...
    axios.get('http://localhost:5000/api/doctor/getAlldoc',config)
...
const handleClose= ()=>{
    ...
    const changeStatus = (e,status,id)=>{  
      ...
      if(status==="accepted"){
       console.log("hey")
        ...
        try{
          axios.post('http://localhost:5000/api/admin/unblockDoctor',{id})
          .then((response)=>{
            console.log(response)
            getDoctors()
            setOpen(null)
  ...
  const getReason =(reason)=>{
    ...
    if(reason){
      setfOpen(false);
      try{
        axios.post('http://localhost:5000/api/admin/blockDoctor',{id:DId,reason})
        .then((response)=>{
          console.log(response)
          getDoctors()
          setOpen(null)
